  ...
  Scenario: Create a service in layer7 gateway using layer7 rest api.
   ...
   """
	    {
			...
			"data": {
				...
				"name": "B2B-Gateway-Automation-Test-API",
				"l7Url": "https://ssgosgtst.isus.emc.com/temp/b2b/ldap*",
				"l7UrlPattern": "/temp/b2b/ldap*",
				"category": "Utilities",
				"backendUrl": "https://cpe-ldapapi.cf.isus.emc.com",
				"enabled": true,
   ...
   """
    ...
    Scenario: Disable an existing service.
     Given The layer7 rest api https://layer7-rest-api.cfcp.isus.emc.com/gateway/services and credentials emFZWEJwZFhObGNnPT09 and emFjR0Z6YzNkdmNtUT09
     When I somnath.upadhyay2@emc.com request to disable the existing service B2B-Gateway-Automation-Test-API on ssgosgtst.isus.emc.com host
    ...
    Scenario: Test a disabled service it should fail. 
     Given The app https://ssgosgtst.emc.com/temp/b2b/ldap/api/persons auth tmp_l7apiuser and tmp_l7apiuser
     When I Search for ntid upadhs5
     Then I get the 500 response code 
    
    Scenario: Enable an existing disabled service.
     Given The layer7 rest api https://layer7-rest-api.cfcp.isus.emc.com/gateway/services and credentials emFZWEJwZFhObGNnPT09 and emFjR0Z6YzNkdmNtUT09
     When I somnath.upadhyay2@emc.com request to enable the existing service B2B-Gateway-Automation-Test-API on ssgosgtst.isus.emc.com host
     Then I should get http status code as 200 and content:
     """
     	{
			"success": true,
    ...
    Scenario: Delete an existing service.
     Given The layer7 rest api https://layer7-rest-api.cfcp.isus.emc.com/gateway/services and credentials emFZWEJwZFhObGNnPT09 and emFjR0Z6YzNkdmNtUT09
     When I somnath.upadhyay2@emc.com request to delete the existing service B2B-Gateway-Automation-Test-API on ssgosgtst.isus.emc.com host
     Then I should get http status code as 200 and content:
     """
     	{
			"success": true,
